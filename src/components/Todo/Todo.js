...
function Todo(props) {
    const { list, deleteItem, clickItem, updateItem, index } = props
  ...
  return (
    <li className='list-card'>
        ...
        <div className='label-icon-group' style={{borderLeft: `4px solid ${colors[index % colors.length].color}`}}>
            <label className={isCompleted ? "label checked" : "label"} htmlFor={id}>{task}</label>
            <div>
                {!isCompleted && <svg style={{marginRight: "5px", cursor: "pointer"}} onClick={() => updateItem(list)} width="19" height="19" fill="#3eac20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <svg style={{marginLeft: "5px", cursor: "pointer"}} onClick={() => deleteItem(id)} width="19" height="19" fill="none" stroke="#f60404"  viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
